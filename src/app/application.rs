...
use super::window_impl::gif_window::GifSelectorWindowImpl;
use super::window_impl::image_window::ImageSelectorWindowImpl;
use super::{
	action::{Action, AppContext, Execute, KeyBinding},
	capture::CaptureDevice,
	event::{KeyInputData, MouseData, UserEvent},
	graphics::Graphics,
	graphics_impl::opengl_impl::GraphicsOpenGLImpl,
	window::{AppWindow, WindowInstance, Target, WindowHashMap, WindowIDDHashMap, EventListener},
};
...
impl ApplicationBuilder {
	...
	fn create_main_window(
		...
		let win_inst = WindowInstance::new(
			windowed_context,
			graphics,
        ...
        let main_win = Box::new(ImageSelectorWindowImpl::new(win_inst));

		window_index
			.entry(AppWindow::RegionSelectorCanvasWindow)
			.or_insert(window_id);
		window.entry(window_id).or_insert(main_win);

	}

    fn create_gif_capture_window(&self, 
		event_loop: &EventLoop<UserEvent>,
		window: &mut WindowHashMap,
		window_index: &mut WindowIDDHashMap,
    ){
		...
		window_index
			.entry(AppWindow::GifSelectorCanvasWindow)
			.or_insert(window_id);
		window.entry(window_id).or_insert(main_win);
    }
	...
	pub fn build(self, event_loop: &EventLoop<UserEvent>) -> std::io::Result<Application> {
        ...
        self.create_gif_capture_window(event_loop, &mut &mut window_hashmap, &mut &mut window_id_hashmap);
...
impl Application {
	...
	pub fn find_window(&mut self, app_window: AppWindow) -> Option<&mut dyn EventListener> {
		if let Some(win) = self.windows_index.get(&app_window) {
			if let Some(mut main_window) = self.windows.get_mut(win) {
